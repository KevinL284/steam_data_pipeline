...
def transform_games_data(data):
    ...
    try:
        ...
        logger.info(
            "Quantidade de jogos recebidos para transformação: %s",
            len(specials)
        )
        ...
        logger.info(
            "Transformação concluída com sucesso. Total de registros: %s",
            len(df)
        )
        ...
        logger.error(
            "Erro de estrutura nos dados recebidos: %s",
            error
        )

    except Exception as error:
        logger.error(
            "Erro inesperado durante transformação dos dados: %s",
            error
        )
